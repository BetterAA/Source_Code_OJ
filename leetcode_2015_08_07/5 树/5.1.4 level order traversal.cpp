...
*/
//注意result是传地址!
void travesal(TreeNode* root, int level, vector<vector<int> > &result){
    ...
    if(root->left!= NULL) travesal(root->left,level+1, result);
    if(root->right!= NULL) travesal(root->right,level+1, result);
}
///迭代版  O(n) O(n)
vector<vector<int> > levelTraversal(TreeNode *root){
    vector<vector<int> >result;
